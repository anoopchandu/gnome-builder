                                   ...
                                   GTask                *task)
{
  ...
  if (NULL != (entries = ide_code_indexer_index_file (indexer,
                                                      ...
                                                      &error)))
    {
      ...
      while (NULL != (entry = g_list_model_get_item (entries, 0)))
        {
          gchar *usr;
          gchar *name;
          IdeSymbolKind kind;
          ...
          usr = ide_code_index_entry_get_usr (entry);
          ...
          g_message ("%s %s %d %d", usr, name + 2, begin_line, begin_line_offset);
          if (usr != NULL)
            ide_persistent_map_builder_insert (map_builder,
                                    usr,
                                    g_variant_new ("(uuuu)",
                                    file_id,
                                    begin_line - 1, begin_line_offset - 1,
                                    flags),
                                        ...
                                        GCancellable *cancellable)
{
  ...
  g_message ("Indexing directory");

  for (int i = 0; i < data->files->len; i++)
    {
  ...
  g_message ("Writing directory index");
                                          ...
                                          gpointer             user_data)
{
  ...
  g_message ("Getting file changes");
...
static void
ide_code_index_builder_index_directories_cb (GObject      *object,
                                             GAsyncResult *result,
                                             gpointer      user_data)
{
  IdeCodeIndexBuilder *self = (IdeCodeIndexBuilder *)object;
...
static void
ide_code_index_builder_get_build_flags_for_files_cb (GObject      *object,
                                                     GAsyncResult *result,
                                                     gpointer      user_data)
{
  ...
  ide_code_index_builder_index_directories_async (self,
                                                  changes,
                                                  cancellable,
                                                  ide_code_index_builder_index_directories_cb,
                                                  g_steal_pointer (&main_task));
}

static void
ide_code_index_builder_get_changes_cb (GObject      *object,
                                       GAsyncResult *result,
                                       gpointer      user_data)
{
  IdeCodeIndexBuilder *self = (IdeCodeIndexBuilder *)object;
  ...
  if (NULL == (changes = ide_code_index_builder_get_changes_finish (self, result, &error)))
    ...
    {
      g_message ("No changes are there, completing task");
      g_task_return_boolean (main_task, TRUE);
      return;
    }
  else if (g_task_return_error_if_cancelled (main_task))
  ...
  /* add time out to finish task */

  ide_build_system_get_build_flags_for_files_async (build_system,
                                                    files,
                                                    cancellable,
                                                    ide_code_index_builder_get_build_flags_for_files_cb,
                                                    g_steal_pointer (&main_task));
}
                                    ...
                                    gpointer             user_data)
{
  ...
  g_message ("Started building index");
  ...
  ide_code_index_builder_get_changes_async (self,
                                            ...
                                            recursive,
                                            cancellable,
                                            ide_code_index_builder_get_changes_cb,
                                            g_steal_pointer (&main_task));
}
