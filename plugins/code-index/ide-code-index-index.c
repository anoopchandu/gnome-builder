...
/* This function will load indexes only if files are not modified */
gboolean
                                        ...
                                        FuzzyMatch        *fuzzy_match)
{
  ...
  g_snprintf (file_id_str, MAX_LEN, "%d", file_id);
  path = dzl_fuzzy_index_get_metadata_string (fuzzy_match->index, file_id_str);
  file = ide_file_new_for_path (context, path);
  location = ide_source_location_new (file, line, line_offset, 0);
                                    ...
                                    const gchar           *key)
{
  ...
  if (!file_id)
    {
      g_message ("symbol location not found");
      return NULL;
    }

  g_snprintf (num, MAX_LEN, "%d", file_id);

  path = dzl_fuzzy_index_get_metadata_string (dir_index->symbol_names, num);
  file = ide_file_new_for_path (ide_object_get_context (IDE_OBJECT (self)), path);

  g_message ("symbol location found, %s %d:%d\n", path, line + 1, line_offset + 1);

  if (flags & IDE_SYMBOL_FLAGS_IS_DEFINITION)
    definition = ide_source_location_new (file, line, line_offset, 0);
  else
    declaration = ide_source_location_new (file, line, line_offset, 0);
