...
/*
 * This class will index a file and returns a GListModel using index entries can be
 * retrieved one by one. For GListModel this will use IdeClangCodeIndexEntry
 * which is backed by TU of the file.
 */
...
/* This will return a GListModel backed by translation unit of file. */
static GListModel *
...
static void
ide_clang_code_indexer_get_translation_unit_cb (GObject       *object,
                                                GAsyncResult  *result,
                                                gpointer       user_data)
{
  ...
  key = ide_clang_translation_unit_get_key (unit, location, &error);
...
static void
ide_clang_code_indexer_get_key_async (IdeCodeIndexer       *indexer,
                                      IdeSourceLocation    *location,
                                      GCancellable         *cancellable,
                                      ...
                                      gpointer              user_data)
{
  ...
  ide_clang_service_get_translation_unit_async (service,
                                                ...
                                                0,
                                                cancellable,
                                                ide_clang_code_indexer_get_translation_unit_cb,
                                                g_steal_pointer (&task));
}

static gchar *
ide_clang_code_indexer_get_key_finish (IdeCodeIndexer  *self,
                                       GAsyncResult    *result,
                                       GError         **error)
{
...
code_indexer_iface_init (IdeCodeIndexerInterface *iface)
{
  ...
  iface->get_key_async = ide_clang_code_indexer_get_key_async;
  iface->get_key_finish = ide_clang_code_indexer_get_key_finish;
}
